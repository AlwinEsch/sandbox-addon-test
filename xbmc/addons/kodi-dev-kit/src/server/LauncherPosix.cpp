...
bool CChildLauncherPosix::Launch(const std::vector<std::string>& argv)
{
  ...
  if (!m_mainReceiver)
  {
    addon::utils::LOG_MESSAGE(ADDON_LOG_ERROR, "CChildLauncherPosix::%s: Main receiver not present", __func__);
    return false;
  }

  if (!m_mainReceiver->Create(true, true))
  {
    addon::utils::LOG_MESSAGE(ADDON_LOG_ERROR, "CChildLauncherPosix::%s: Failed to create receiver", __func__);
    return false;
  }

  if (!m_mainTransmitter->Create(true))
  {
    addon::utils::LOG_MESSAGE(ADDON_LOG_ERROR, "CChildLauncherPosix::%s: Failed to create transmitter", __func__);
    return false;
  }
  ...
  if (m_otherThreadReceiver)
  {
    ...
    {
      addon::utils::LOG_MESSAGE(ADDON_LOG_ERROR, "CChildLauncherPosix::%s: Failed to create other thread receiver", __func__);
      return false;
    }
  }
  ...
  if (pid != 0)
  {
    ...
    addon::utils::LOG_MESSAGE(ADDON_LOG_DEBUG, "CChildLauncherPosix::%s: Child fork time %f", __func__, fork_time.count());
  }

  if (pid == -1)
  {
    addon::utils::LOG_MESSAGE(ADDON_LOG_ERROR, "CChildLauncherPosix::%s: Failed to create child process");
    return false;
  }
  ...
  if (!StartObserver())
  {
    addon::utils::LOG_MESSAGE(ADDON_LOG_INFO, "CChildLauncherPosix::%s: Failed to start sandbox process observer", __func__);
    Kill(true);
    return false;
  }
...
bool CChildLauncherPosix::Kill(bool wait)
{
  if (m_pid)
  {
    if (!StopObserver())
    {
      addon::utils::LOG_MESSAGE(ADDON_LOG_INFO, "CChildLauncherPosix::%s: Failed to stop sandbox process observer", __func__);
    }
    ...
    if (m_lastStatus == ChildStatus::Running)
    {
      ...
      if (!did_terminate)
      {
        addon::utils::LOG_MESSAGE(ADDON_LOG_ERROR, "CChildLauncherPosix::%s: Unable to terminate process %i", __func__, m_pid);
        return false;
      }
    }
  }
